function [cell_miti_data]=calculate_and_plot_pathloss_coordination_rev1(app,data_label1,array_mitigation,required_pathloss,min_full_pl_data,base_protection_pts,sim_array_list_bs,grid_spacing,string_prop_model)

                    num_miti=length(array_mitigation)
                    margin_dB=required_pathloss-min_full_pl_data;

                    cell_miti_data=cell(num_miti,4);
                    %%%%%%%%%1)Mitigation
                    %%%%%%%%%2) required_pathloss
                    %%%%%%%%3) max_knn_dist
                    %%%%%%%4) Convex Hull
                    for miti_idx=1:1:num_miti
                        ...
                        if ~isempty(idx_keep)
                            ...
                            cell_miti_data{miti_idx,1}=miti_dB;
                            cell_miti_data{miti_idx,2}=required_pathloss;
                            cell_miti_data{miti_idx,3}=max_knn_dist;
                            cell_miti_data{miti_idx,4}=convex_bound;
                            ...
                            if ~isempty(non_inf_keep_grid_pts)==1
                                ...
                                dbm2_range=ceil(max(non_inf_round_dbm_data))-floor(min(horzcat(min(non_inf_round_dbm_data),0)));
                                dbm2_range
                                if dbm2_range>0
                                    ...
                                    pause(0.1)
                                    filename1=strcat('Convex_Bound','_',data_label1,'_',string_prop_model,'_',num2str(grid_spacing),'km_',num2str(miti_dB),'dB.png');
